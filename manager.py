...
# 插入调度任务
scheduler.add_job(func=update_database, trigger='cron', id='scheduler_data', hour=0, minute=55)
...
def InitDataBase():
    db.create_all()
    Role.insert_roles()
    Admin = User(email=app.config['FLASK_ADMIN'], password='w123456', username='GarryLin-Admin', confirmed=True)
